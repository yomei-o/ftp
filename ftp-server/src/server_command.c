...
t_command_alias server_commands[50]
    = { { "USER", HELP_USER, &server_command_user },
          ...
          { "XPWD", HELP_PWD, &server_command_pwd },
          { "PWD", HELP_PWD, &server_command_pwd },
          { "LIST", HELP_LIST, &server_command_list },
          { "NLST", HELP_NLST, &server_command_list },
...
{
    char** arguments;
    char buf[sizeof(client_info->cwd)];
    ...
    if (arguments[0][0] == '/') {
        snprintf(buf,sizeof(client_info->cwd), "%s/%s", client_info->root_dir, *arguments);
    }else{
        snprintf(buf, sizeof(client_info->cwd), "%s/%s", client_info->cwd, *arguments);
    }
...
int server_command_retr(t_client_info* client_info, char* argument)
{
    ...
    if ((fd = fopen(file_name_buf,"rb")) == NULL) {
    ...
    int ret = 0;
    char buffer[40960];
    while ((ret = fread( buffer,1, 40960,fd)) > 0) {
        (void)send(client_info->data.fd, buffer, ret,0);
    }
    fclose(fd);
...
int server_command_stor(t_client_info* client_info, char* argument)
{
    ...
    if ((fd = fopen(file_name_buf, "wb")) == NULL) {
        perror("File open failed");
        if (server_command_list_close(client_info) == -1) {
            return (-1);
        }
    ...
    int ret = 0;
    char buffer[40960];
    while ((ret = recv(client_info->data.fd, buffer, 40960,0)) > 0) {
...
int server_command_list_loop(t_client_info* client_info, char* argument)
{
    ...
    return (0);
#else
    char stdout_[4096*10];
    list_directory(client_info->cwd,stdout_, sizeof(stdout_));
    REQUEST_RESPONSE(client_info->data.fd, SINGLE_STRING, stdout_);
    return 0;
#endif
}
